...
if [[ "$API_TYPE" == "REST" ]]; then
  ...
  create_method () {
    ...
    aws apigateway put-integration \
      ...
      --uri arn:aws:apigateway:$AWS_REGION:lambda:path/2015-03-31/functions/$AUTH_FUNCTION_ARN/invocations >/dev/null 2>&1 || true
  ...
  DEPLOY_ID=$(aws apigateway create-deployment --rest-api-id "$API_ID" --region "$AWS_REGION" --stage-name "$STAGE_NAME" --description "Auth deploy $(date)" --query 'id' --output text)
